...
function navigate(page, params = {}) {
  ...
  const pages = { dashboard, analytics, projects, selection, wbs, evm, notifications, reports, users, gantt, members, profile, projectInfo };
...
async function users() {
  ...
  try {
    ...
    document.getElementById('users-tbody').innerHTML = data.map(u => `<tr>
      ...
      <td><span class="badge badge-${roleColor[u.role] || 'gray'}">${u.role}</span></td>
...
async function members() {
  ...
  try {
    const [projs, allUsers] = await Promise.all([
      api('GET', '/api/projects'),
      CURRENT_USER?.role === 'Admin' ? api('GET', '/api/users') : Promise.resolve([]),
    ]);
    window._allProjects = projs;
...
async function openMemberModal() {
  ...
  if (!window._allUsers?.length) {
    try { window._allUsers = await api('GET', '/api/users'); } catch {}
  }
